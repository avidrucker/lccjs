/*
 * donut_ref.c — a1k0n's integer / bit-ops donut (the "golden" reference).
 *
 * Source: https://www.a1k0n.net/code/donutbitops.c.html
 * Saved verbatim for issue #1468 as the parity reference for a base-LCC port.
 *
 * Key properties that make it portable to the 16-bit integer LCC ISA:
 *   - no floating point, no libm sin/cos/sqrt at runtime
 *   - HAKMEM-149 Minsky circle for rotation, CORDIC for magnitude + lighting
 *   - fixed-point (~14 bit) arithmetic, 16-bit intermediates
 * This original animates forever (R(...) rotation + usleep + cursor-up).
 * Our deliverable is a SINGLE static frame, so the animation/timing/ANSI parts
 * will be dropped in the simplified variant.
 */
#include <stdint.h>
#include <stdio.h>
#include <string.h>
