...
// #1184 + #1193: notice-not-prevent. A new model (or a non-canonical one) is recorded,
// not rejected — models are an open-growth list, so an unrecognised value is far
// more likely a legitimately-new release (or experimental label like "owl-alpha")
// than an error worth aborting on. We still emit a one-line notice so a human
// can see it and bless/normalise it. (Roles below stay a hard reject.)
...
if (input.model != null && input.model !== '' && !CANONICAL_MODEL.test(String(input.model))) {
  console.error(
    ...
    `(canonical form is <family>-<major.minor> or <family>-<label> for experimental, e.g. opus-4.8 or owl-alpha) — recording it anyway`);
}
