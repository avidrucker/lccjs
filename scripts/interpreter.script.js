// interpreter.script.js
// TODO: how to run this test (see test/interpreterBattery.script.js for example)
  ...
  try {
    ...
    if (!skipCache) {
      ...
      if (cacheValid) {
        ...
        if(!testResult) {
          // Test failed with cached output
          // update the .lst cache with Docker
          console.log('Updating cache with Docker output...');
        } else {
          // Test passed with cached output
          console.log("Test passed with cached output. No Docker run needed.");
          process.exit(0);
        }
      }
    } else {
