...
| `model` | string / empty | the model (Claude or experimental/other) used for the work, in short form `<family>-<major>.<minor>` or `<family>-<label>` (e.g. `sonnet-4.6`, `opus-4.8`, `owl-alpha`). Empty for rows logged before the column was added (#275). Do not fabricate; leave blank when genuinely unknown. Added as a side-effect of #275; documented in #314; extended for non-numeric labels in #1193. |
