...
- [ ] document worktree/claim naming + mandatory `--as` for concurrent fan-out, correct the "branch namespace is the source of truth" claim, and add the `[A-Za-z0-9._-]`-only naming rule that keeps the pdd-gate excludes working (#195 — WRITER; folds #232)
  <!-- @todo #195:30m/WRITER worktree/claim naming + --as fan-out docs (folds #232): (a) claude_workflow.md: bare `auto` is first-claim-of-a-solo-session only, concurrent fan-out pre-assigns `--as <fruit>`; correct claim.js header (l.38-39) + design-agent-worktree-identity.md (l.36 + l.70-74) — takenFruits() is worktree-scoped, not branch-namespace; (b) document the [A-Za-z0-9._-]-only worktree/claim naming rule — no `/`→`+`, which silently disables pdd-gate excludes (#224), workaround validated in #226. Cross-link docs/research/claim-fruit-session-scope.md. See #195 (post-#212 line refs sharpened by #223/R3; #232 folded in via #235) -->
...
- [ ] reconcile agent-identity precedence to one canonical source — restated in 6 places, drifted after #212 (#230 — WRITER)
  <!-- @inprogress #230:45m/WRITER make docs/design-agent-worktree-identity.md the single home for the `--as` > CLAUDE_AGENT_NAME > auto precedence + race model; reduce the claim.js header + claude_workflow.md restatements to a one-line summary + link (repo-scoped; the 2 memories + puzzle-velocity skill are out-of-repo, reconciled separately); fold in the "sync main before claiming" note from #223. Decompose if >60m. See #230 and docs/research/agent-identity-guidance-reconciliation.md -->
- [ ] velocity `agent`-column doc says "worktree fruit name", contradicting the terminal-name convention (#229 — WRITER)
  <!-- @inprogress #229:15m/WRITER docs/puzzle-velocity.md agent-column (~l.30) says "the worktree fruit identity, uppercased" — reword to "the human/terminal name the agent runs under, which equals the worktree fruit once CLAUDE_AGENT_NAME/--as is set; divergence only under bare auto", cross-link design-agent-worktree-identity.md. Same wording also in the out-of-repo puzzle-velocity skill. See #229 -->
