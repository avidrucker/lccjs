...
- [ ] lock in `.a`-path LST source-text parity vs oracle (mnemonics + same-line + standalone comments), whitespace-lenient
  <!-- @inprogress #155:45m/DEV add a whitespace-lenient TDD parity test: lccjs lcc.js vs oracle on a comment-rich .a (e.g. demos/demoQ.a); normalize banner/date + whitespace; assert every source line (incl. comments) survives in the .lst. Tolerate deviation 11 (oracle truncates, lccjs full). Proven in #145. See #155 -->
