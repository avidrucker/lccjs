...
1. Read the open issues and any supporting state that affects ranking.
2. Partition issues into actionable, blocked, iceboxed, and human-routing groups.
3. Rank actionable issues by severity first, then Yegor priority, then issue number.
...
1. Severity
2. Yegor priority
3. Issue number as a stable tie-breaker

Keep the ordering explicit so the human can see why one ticket comes before another.
