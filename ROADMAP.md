...
- **Playground** (deployed at `/showcase/`, generated by `scripts/build-site.js` → `docs/site/showcase/index.html`) — edit-assemble-run loop with live syntax highlighting as you type LCC assembly in the browser; uses the Lezer `lcc()` LanguageSupport plus the custom TextMate grammar for the highlighted preview alongside the editor
